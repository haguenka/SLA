...
with tab2:
    ...
    df_top_rm = top_medicos_rm.reset_index().rename(columns={"index": "MEDICO_SOLICITANTE": "Quantidade"})
    ...
    df_top_rm["Exames"] = df_top_rm["MEDICO_SOLICITANTE"].apply(get_exam_breakdown)
    ...
    # Cria DataFrame com nomes e quantidade
    df_top_tc = top_medicos_tc.reset_index().rename(columns={"index": "MEDICO_SOLICITANTE": "Quantidade"})
    # Adiciona o detalhamento dos exames para cada médico em TC
    df_top_tc["Exames"] = df_top_tc["MEDICO_SOLICITANTE"].apply(
        lambda medico: "\n".join([
            f"{exame} - {count}" 
